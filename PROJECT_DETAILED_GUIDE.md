...
DocuAsk 是一个本地文档 RAG 问答系统。用户上传 `.txt` 或 `.md` 文档后，系统会把文档切分成片段，转换成向量并写入 Chroma 向量数据库；用户提问时，系统先从本地文档中检索相关片段，再把片段交给大模型生成回答，并返回答案来源。
...
当前不适合直接包装成生产系统，因为它还没有多用户权限、PDF/Word 解析、线上部署、权限隔离、日志监控和大规模压测。
...
注意：这个结果只说明当前小型 FAQ 样例和固定问题集下的表现，不能代表所有文档场景。
...
1. 扩大评测集，记录失败案例。
2. 接入 rerank，提高 Top-1 排序质量。
3. 增加 PDF / Word 文档解析。
4. 增加 Docker Compose，提升部署复现能力。
5. 增加日志和错误分类。
6. 增加多文档管理能力。
7. 增加用户权限和知识库隔离。

优先级最高的是评测集和 rerank，因为 RAG 系统最核心的问题不是页面做得多复杂，而是能否稳定检索到正确资料。
...
```text
DocuAsk 是我实现的一个本地文档 RAG 问答系统。它支持上传 txt 和 Markdown 文档，系统会把文档切分成 chunks，转换成 embedding 后写入 Chroma。用户提问时，系统先用 vector、BM25 或 RRF 检索相关 chunks，再把检索上下文交给 DeepSeek 生成回答，并展示来源 chunk。

项目中我重点做了三件事：第一，跑通从文档上传到来源引用的完整 RAG 链路；第二，把 chunking、embedding、retrieval、generation、evaluation 等逻辑拆成后端模块，并用 FastAPI 暴露接口；第三，用 Top-1 hit 和 Top-k recall 对检索效果做小样本评测，并用 pytest 覆盖核心接口和异常分支。

当前它仍然是本地原型，不是生产级系统，还不支持 PDF、Word、多用户权限和大规模压测。后续我会优先扩展评测集、接入 rerank，并补充复杂文档解析能力。
```
