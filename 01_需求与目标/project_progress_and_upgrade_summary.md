...
**当前阶段**：本地 RAG 原型完成，准备进入 v2 工程化升级  
**当前最新提交**：`b31260a Add day 36 DocuAsk v2 upgrade baseline`

---

## 1. 项目当前定位

DocuAsk 当前是一个本地文档 RAG 问答原型，用于验证完整 RAG 链路：

```text
文档上传 -> 文档读取 -> 文档切分 -> embedding -> Chroma 检索 -> 上下文组装 -> LLM 回答 -> 来源引用
```
...
- Streamlit 单体应用，后端服务边界还不清晰。
- Chroma 当前使用临时 collection，服务重启后知识库不复用。
- 当前评测集只有 10 个固定问题，规模较小。
- 当前主要是向量检索，未做 BM25 / RRF / rerank。
- 当前截图和 README 能展示项目，但工程化程度还可以提升。
