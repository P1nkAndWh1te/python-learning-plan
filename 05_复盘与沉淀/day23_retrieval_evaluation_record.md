...
Day25 将固定评测问题扩展到 10 个，并为每个问题增加 `expected_top_chunk` 和 `hit` 字段。
...
这说明教学版关键词 embedding 对宽泛问题和相近 API/Key 主题仍然区分不足。
...
1. 将 10 题评测结果整理成独立表格。
2. 为未命中问题分析失败原因。
3. 替换为真实语义 embedding 后重新对比。
4. 记录教学版 embedding 和真实 embedding 的命中率变化。
5. 形成最终项目评测报告。
```
